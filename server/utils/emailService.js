...
class EmailService {
  ...
  async sendPasswordResetEmail(user, resetToken, resetUrl) {
    ...
    const emailContent = {
      from: `"${settings.SMTP_FROM_NAME || 'Agila'}" <${settings.SMTP_FROM_EMAIL}>`,
      to: user.email,
      subject: `Password Reset - ${settings.SITE_NAME || 'Agila'}`,
      text: `Hi ${user.first_name} ${user.last_name},

You requested a password reset for your Agila account.
...
The Agila Team`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <p>You requested a password reset for your <strong>Agila</strong> account.</p>
          ...
          <p>Best regards,<br><strong>The Agila Team</strong></p>
        </div>
      `
    };
